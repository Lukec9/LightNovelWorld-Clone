...
const NovelComment = ({ comment, setComments }) => {
  const {
    state: { user: user },
  } = useAuthContext();
  ...
  return (
    <>
      <div className="comment-item  none">
        ...
        <div className="comment-body">
          <meta itemProp="dateCreated" content={comment.createdAt} />
          <div className="comment-text" itemProp="text">
            <p>{comment.text}</p>
          </div>
          <div className="toolbar">
            <span className="_tl">
              <a className="reply" data-uaction="comreply">
                <svg
            ...
            <span className="usrlike">
              <span className="_grp" onClick={() => handleLikeDislike("like")}>
                <input
                  ...
                  type="radio"
                  name={`ulike_${comment._id}`}
                  defaultValue={0}
                />
                <label
                  className={liked ? "active" : ""}
                  htmlFor={`inputlike_${comment._id}`}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                  ...
                  <span className="lc_{comment._id}">{likeCount}</span>
                </label>
              </span>
              ...
              >
                ...
                <label
                  ...
                  htmlFor={`inputdislike_${comment._id}`}
                >
                  <span className="dlc_{comment._id}">{dislikeCount}</span>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    ...
                    viewBox="0 0 16 16"
                  >
                    <path d="M8.864 15.674c-.956.24-1.843-.484-1.908-1.42-.072-1.05-.23-2.015-.428-2.59-.125-.36-.479-1.012-1.04-1.638-.557-.624-1.282-1.179-2.131-1.41C2.685 8.432 2 7.85 2 7V3c0-.845.682-1.464 1.448-1.546 1.07-.113 1.564-.415 2.068-.723l.048-.029c.272-.166.578-.349.97-.484C6.931.08 7.395 0 8 0h3.5c.937 0 1.599.478 1.934 1.064.164.287.254.607.254.913 0 .152-.023.312-.077.464.201.262.38.577.488.9.11.33.172.762.004 1.15.069.13.12.268.159.403.077.27.113.567.113.856s-.036.586-.113.856c-.035.12-.08.244-.138.363.394.571.418 1.2.234 1.733-.206.592-.682 1.1-1.2 1.272-.847.283-1.803.276-2.516.211a10 10 0 0 1-.443-.05 9.36 9.36 0 0 1-.062 4.51c-.138.508-.55.848-1.012.964zM11.5 1H8c-.51 0-.863.068-1.14.163-.281.097-.506.229-.776.393l-.04.025c-.555.338-1.198.73-2.49.868-.333.035-.554.29-.554.55V7c0 .255.226.543.62.65 1.095.3 1.977.997 2.614 1.709.635.71 1.064 1.475 1.238 1.977.243.7.407 1.768.482 2.85.025.362.36.595.667.518l.262-.065c.16-.04.258-.144.288-.255a8.34 8.34 0 0 0-.145-4.726.5.5 0 0 1 .595-.643h.003l.014.004.058.013a9 9 0 0 0 1.036.157c.663.06 1.457.054 2.11-.163.175-.059.45-.301.57-.651.107-.308.087-.67-.266-1.021L12.793 7l.353-.354c.043-.042.105-.14.154-.315.048-.167.075-.37.075-.581s-.027-.414-.075-.582c-.05-.174-.111-.273-.154-.316l-.353-.353.353-.353c.046-.046.104-.174.001-.48-.263-.423-.591-.747-1.009-.962l-.355-.355.355-.354c.045-.046.115-.187.112-.411a.9.9 0 0 0-.12-.416c-.165-.288-.502-.559-1.066-.559z" />
                  </svg>
                </label>
              </span>
            </span>

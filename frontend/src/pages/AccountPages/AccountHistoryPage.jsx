  ...
  useEffect(() => {
    const fetchUserHistory = async () => {
      try {
        ...
        const response = await axiosInstance.get("/users/recently-read");
        setNovels(response.data);

        // Fetch progress for each novel
        const novelsWithProgress = await Promise.all(
          response.data.map(async novel => {
            const {
              data: { progress },
            } = await axiosInstance.get(`/novels/${novel._id}/progress/`);
            return {
              ...novel,
              progress,
            };
              ...
              {novels &&
                novels.map((n, i) => (
                  <Suspense key={i} fallback={<Spinner />}>
                    <HistoryNovelCard novel={n} key={n._id} />
                  </Suspense>
                ))}
            </div>
          </section>
